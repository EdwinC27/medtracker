/* Dashboard: next dose, next appointment, ending soon, today's doses,
   active medications. Refreshes itself every minute so the countdown and the
   dose statuses stay honest without a page reload. */
(function () {
  'use strict';
  ...
  async function render() {
    ...
    document.getElementById('dashboard-date').textContent = F.dateLong(new Date());
  ...
  function renderTodaysDoses(data) {
    ...
    list.textContent = '';
    summary.textContent = T.t('dashboard.doses_taken_today', {
      taken: data.todays_summary.taken, total: data.todays_summary.total,
    });

    if (!data.todays_doses.length) {
      list.appendChild(UI.emptyState('dashboard.todays_doses_empty'));
      return;
    }
    data.todays_doses.forEach(function (dose) {
      list.appendChild(C.doseRow(dose, render));
    });
  }
